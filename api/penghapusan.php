...
if ($method === 'GET') {
  ...
  if ($id > 0) {
    $hdr = db_one("SELECT id, id_gudang FROM penghapusan WHERE id = ? LIMIT 1", [$id]);
    if (!$hdr) respond(404, ['error' => 'Penghapusan tidak ditemukan']);
...
if ($method === 'POST') {
  ...
  if ($type === 'master') {
    $stmt = db()->prepare(
      "INSERT INTO penghapusan (no_hapus,tanggal,alasan,id_admin,id_gudang)
       VALUES (?,?,?,?,?)"
    );
    $stmt->execute([
      (string)($b['no_hapus'] ?? ''),
    ...
    audit_log($me['user_id'], 'CREATE', 'penghapusan', $id, ['id_gudang' => $me['id_gudang']]);

    respond(201, ['id' => $id, 'message' => 'Berhasil dibuat']);
  }

  if ($type === 'detail') {
    $id_hapus = (int)($b['id_hapus'] ?? 0);
    $id_batch = (int)($b['id_batch'] ?? 0);
    $jumlah = (float)($b['jumlah'] ?? 0);

    if ($id_hapus <= 0 || $id_batch <= 0 || $jumlah <= 0) {
      respond(400, ['error' => 'Field wajib: id_hapus,id_batch,jumlah']);
    }

    $hdr = db_one("SELECT id, id_gudang FROM penghapusan WHERE id = ? LIMIT 1", [$id_hapus]);
    if (!$hdr) respond(404, ['error' => 'Penghapusan tidak ditemukan']);
    if ((int)$hdr['id_gudang'] !== $me['id_gudang']) respond(403, ['error' => 'Forbidden']);
    ...
    try {
      ...
      $stmt = $pdo->prepare("INSERT INTO penghapusan_detail (id_hapus,id_batch,jumlah) VALUES (?,?,?)");
      $stmt->execute([$id_hapus, $id_batch, $jumlah]);

      $stmt = $pdo->prepare("UPDATE stok_batch SET stok = stok - ? WHERE id_gudang=? AND id_batch=?");
      $stmt->execute([$jumlah, $me['id_gudang'], $id_batch]);

      audit_log($me['user_id'], 'ADD_ITEM', 'penghapusan', $id_hapus, [
        'id_batch' => $id_batch,
        'qty' => $jumlah,
      ]);
