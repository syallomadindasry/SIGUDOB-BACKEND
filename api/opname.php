...
if ($method === 'GET') {
  ...
  if ($id > 0) {
    ...
    if (!$hdr) respond(404, ['error' => 'Opname tidak ditemukan']);
...
if ($method === 'POST') {
  ...
  if ($type === 'master') {
    ...
    audit_log($me['user_id'], 'CREATE', 'opname', $id, ['id_gudang' => $me['id_gudang']]);
  ...
  if ($type === 'detail') {
    ...
    if ($opnameId <= 0 || $idBatch <= 0) respond(400, ['error' => 'opname_id dan id_batch wajib']);
    ...
    $row = db_one("SELECT stok FROM stok_batch WHERE id_gudang=? AND id_batch=? LIMIT 1", [$me['id_gudang'], $idBatch]);
    ...
    audit_log($me['user_id'], 'ADD_ITEM', 'opname', $opnameId, [
      'id_batch' => $idBatch,
...
if ($method === 'PUT') {
  ...
  if ($action === 'approve') {
    ...
    try {
      ...
      audit_log($me['user_id'], 'APPROVE', 'opname', $id, []);
  ...
  if ($action === 'reject') {
    ...
    audit_log($me['user_id'], 'REJECT', 'opname', $id, ['note' => (string)($b['note'] ?? '')]);
